...
class L1_plus_perceptualLoss(nn.Module):
    def __init__(self, lambda_L1, lambda_perceptual, perceptual_layers, gpu_ids, percep_is_l1, select_loss):
    ...
    def forward(self, inputs, targets):
        ...
        # normal L1
        if self.select_loss == 1:
            loss_l1 = F.l1_loss(inputs, targets) * self.lambda_L1
        elif self.select_loss == 2:
            loss_l1 = F.mse_loss(inputs, targets) * self.lambda_L1
        if self.select_loss == 3:
            loss_l1 = (F.l1_loss(inputs, targets) + F.mse_loss(inputs, targets)) * self.lambda_L1/2
        else:
            loss_l1 = F.smooth_l1_loss(inputs, targets)*self.lambda_L1
        ...
        if inputs.shape[1]==6:
            mean = torch.tile(mean,(1,2,1,1))
            std = torch.tile(std,(1,2,1,1))
        ...
        if inputs.shape[1]==6:
            fake_p2_norm = torch.cat((self.vgg_submodel(fake_p2_norm[:,:3,:,:]), self.vgg_submodel(fake_p2_norm[:,3:,:,:])), axis=1)
            input_p2_norm = torch.cat((self.vgg_submodel(input_p2_norm[:,:3,:,:]), self.vgg_submodel(input_p2_norm[:,3:,:,:])), axis=1)
        else:
        ...
        elif self.percep_is_l1 == 2:
            # use l2 for perceptual loss
            loss_perceptual = F.mse_loss(fake_p2_norm, input_p2_norm_no_grad) * self.lambda_perceptual

        else:
            loss_perceptual = F.smooth_l1_loss(fake_p2_norm, input_p2_norm_no_grad) * self.lambda_perceptual
